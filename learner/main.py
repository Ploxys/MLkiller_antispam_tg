...
neuro_fighter = Pipeline([
    ('tfidf', TfidfVectorizer()),
    ('scaler', StandardScaler(with_mean=False)),
    ('clf', MLPClassifier(activation="logistic",max_iter=128,learning_rate="invscaling",n_iter_no_change=20,hidden_layer_sizes=[(1024),(1024)]))])
apofes = Pipeline([
    ('tfidf', TfidfVectorizer()),
    ('clf', RidgeClassifier())
])
